...
const $searchBtn = document.querySelector('.search-submit') as HTMLButtonElement;
const $resetBtn = document.querySelector('.search-reset') as HTMLButtonElement;
const $artistsOrAlbums = document.querySelectorAll('.grid-items') as NodeListOf<HTMLOListElement>;
const $trackTable = document.querySelector('.tracklist') as HTMLTableElement;
const $moreLink = document.querySelectorAll('.more-link') as NodeListOf<HTMLParagraphElement>;
const $moreLinkLink = document.querySelectorAll('.more-link-link') as NodeListOf<HTMLLinkElement>;
const $secondarySectionLink = document.querySelectorAll('.secondary-nav-item-link') as NodeListOf<HTMLLinkElement>;
const $infoSection = document.querySelectorAll('.search-info-section') as NodeListOf<HTMLElement>;
const $notFoundMes = document.querySelectorAll('.message') as NodeListOf<HTMLParagraphElement>;
const $heading = document.querySelector('.content-top-header') as HTMLHeadingElement;

const apiRoot1: string =  "http://ws.audioscrobbler.com/2.0/";
const apiKey1: string = "80bd75ebdb197778667b6757f51d855d";
 ...
 */
 function handlerSearchErrors(e: string, mesForUser: boolean = true){
    console.error(e);
    if(mesForUser)
...
async function dataProcessing() {
    ...
    const tracksDuration = await Promise.all(tracks.map(async (track: Info) =>
        await fetchAdditionalInfoAboutTrack(track.name, track.artist).catch((error) => handlerSearchErrors(error, false))
    ));
 ...
 * @returns {void} ничего не возвращает
 */
function addSearchArtistToUI(artist: Artist){
    const template = `
    <li class="grid-item">
 ...
 * @returns {void} ничего не возвращает
 */
function addSearchAlbumToUI(album: Info){
 ...
 * @returns {void} ничего не возвращает
 */
function addSearchTrackToUI(track: Info) {
 ...
 */
async function fetchArtistsBySearchValue(value: string) {
 ...
 */
async function fetchAdditionalInfoAboutTrack(track: string, artist: string) {
 ...
 * @returns {String} возвращает строку
 */
function getSavedValue(id: string){
