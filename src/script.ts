// @ts-ignore
const $hotRightNowList = document.querySelector('.music-artists') as HTMLOListElement;
const $popularTracksList = document.querySelector('.column-tracks') as HTMLOListElement;
const apiRoot: string =  "http://ws.audioscrobbler.com/2.0/";
const apiKey: string = "80bd75ebdb197778667b6757f51d855d";
 ...
 */
function addHotArtistToUI(artist: Artist) {
    const template = `
 ...
 * @returns {void} ничего не возвращает
 */
function addPopularTrackToUI(track: Track) {
    const template = `
...
async function fetchTagsByArtistName(name: string) {
    ...
    return data.toptags.tag.slice(0,3) as Tag[];
}
 ...
 * @returns {void} ничего не возвращает.
 */
function handlerErrors(e: string, mesForUser: boolean = true){
    console.error(e);
    if(mesForUser)
...
async function main() {
    ...
    const artistTags = await Promise.all(hotArtists.map(async (hotArtist: Artist) =>
        await fetchTagsByArtistName(hotArtist.name).catch((error) => handlerErrors(error, false))
    ));
    ...
    const tracksTags = await Promise.all(popularTracks.map(async (popularTrack: Track) => 
        await fetchTagsByTrack(popularTrack.name, popularTrack.artist.name).catch((error) => handlerErrors(error, false))
    ));
